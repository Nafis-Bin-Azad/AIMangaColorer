...
class MangaLibrary:
    ...
    def __init__(self, library_dir: Path, data_file: Path):
        """
        Initialize manga library.
        
        Args:
            library_dir: Root library directory (contains manga folders with original/ and colored/)
            data_file: JSON file for storing progress and bookmarks
        """
        self.library_dir = Path(library_dir)
        self.downloads_dir = self.library_dir  # Alias for compatibility
    ...
    def scan_library(self) -> List[MangaEntry]:
        ...
        if not self.library_dir.exists():
            logger.info("Library directory does not exist")
            return manga_list
        
        # Each manga is a folder in library/ with original/ and colored/ subfolders
        for manga_dir in self.library_dir.iterdir():
            if not manga_dir.is_dir():
                continue
            
            try:
                original_dir = manga_dir / "original"
                if not original_dir.exists():
                    continue
                
                # Find chapters (folders starting with Ch_ in original/)
                chapters = sorted([
                    d.name for d in original_dir.iterdir()
    ...
    def has_colored_version(self, manga_title: str, chapter: str = None) -> bool:
        ...
        """
        colored_base_path = self.library_dir / manga_title / "colored"
        ...
        # Check if any colored files exist
        return any(
            f.suffix.lower() in self.SUPPORTED_EXTENSIONS
    ...
    def get_chapter_pages(self, manga_title: str, chapter: str, use_colored: bool = False) -> List[Path]:
        ...
        if use_colored:
            colored_path = self.library_dir / manga_title / "colored" / chapter
        ...
        chapter_path = self.library_dir / manga_title / "original" / chapter
