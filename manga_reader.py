...
class MangaReaderFrame(ttk.Frame):
    ...
    def _setup_keyboard_shortcuts(self):
        ...
        self.bind('<plus>', lambda e: self.set_zoom(self.zoom_level + 0.1))
        self.bind('<equal>', lambda e: self.set_zoom(self.zoom_level + 0.1))  # + without shift
        self.bind('<minus>', lambda e: self.set_zoom(self.zoom_level - 0.1))
        
        self.bind('f', lambda e: self.set_fit_mode("width"))
        self.bind('h', lambda e: self.set_fit_mode("height"))
        self.bind('0', lambda e: self.set_fit_mode("actual"))
        
        self.bind('b', lambda e: self.toggle_bookmark())
        self.bind('t', lambda e: self.toggle_thumbnails())
        
        self.bind('<Escape>', lambda e: self.close_reader())
    ...
    def close_reader(self):
        ...
        if hasattr(self.master, '_recreate_library_ui'):
            self.master._recreate_library_ui()
        elif hasattr(self.master, 'refresh_library'):
            # Fallback for old code
            self.master.refresh_library()
